...
class Travel
{
    ...
    static function extract($data): array
    {
        ...
        foreach ($boardingCards as $card) {
            $points[] = self::extractPoints($card);
        }
        ...
        while (count($boardingCards) > 0) {
            ...
            foreach ($boardingCards as $key => $card) {
                if (str_contains($card, self::extractPoints($lastCard)["end"])) {
                    $sortedCards[] = $card;
                    unset($boardingCards[$key]);
                    break;
                }
    ...
    static function extractPoints($card)
    {
